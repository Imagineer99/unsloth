...
LATEST = "91f975068fbbb93b9b08135185d7ad4abb0bdb29"
...
def studio(repo):
    ...
    nodes = [
        n
        for n in ast.walk(proof.tree(path))
    ...
    spec = importlib.util.spec_from_file_location(
        "core.inference.chat_template_helpers", helper_path
    )
    ...
    proof.module(
        "utils.datasets", {"MODEL_TO_TEMPLATE_MAPPER": {}, "get_tokenizer_chat_template": None}
    )
    scope = {
        "Optional": Optional,
        "Generator": Generator,
        "logger": logging.getLogger("fix-proof"),
        "trailing_assistant_text": lambda messages: messages[-1]["content"]
    ...
    defaults = {
        ...
        for a, d in zip(stream.args.args[-len(stream.args.defaults) :], stream.args.defaults)
    }
    ...
    def chat(
        ...
        def capture(prompt, *args, **kwargs):
            yield {
                "prompt": prompt,
                "ids": encode(tok, prompt, **kwargs),
                "specials": kwargs.get(
                    "add_special_tokens", defaults.get("add_special_tokens", True)
                ),
            }

        backend = types.SimpleNamespace(
            active_model_name = "local-export",
            models = {"local-export": {"tokenizer": processor}},
            _normalize_top_k = lambda x: x,
            _apply_chat_template_for_generation = render,
            generate_stream = capture,
            format_chat_prompt = lambda *a, **k: "Hello",
        )
        result = list(
            scope["_generate_chat_response_inner"](
                backend, messages, continue_final_message = messages[-1]["role"] == "assistant"
            )
        )
        assert len(result) == 1
        return result[0]
...
def probe(args):
    ...
    processor = proof.load_boundary(
        repo, AutoProcessor.from_pretrained(snapshot, local_files_only = True), config
    )
    ...
    processor = proof.load_boundary(
        repo, AutoProcessor.from_pretrained(output / "export", local_files_only = True), config
    )
    exported = chat(processor, messages)
    assert direct["ids"] == exported["ids"]
    template_ids = processor.tokenizer.apply_chat_template(
        messages, tokenize = True, add_generation_prompt = True
    )
    template_ids = template_ids["input_ids"] if hasattr(template_ids, "keys") else template_ids
    has_template_bos = template_ids[0] == processor.tokenizer.bos_token_id
    cases = []
    ...
    for template_index, template in enumerate(templates):
        ...
        for text in ["Hello", "", "你好 🦥", "<bos> inside text", "two\nlines"]:
            for continuation in [False, True]:
                ...
                expected = processor.tokenizer(result["prompt"], add_special_tokens = False)[
                    "input_ids"
                ]
                cases.append(
                    {
    ...
    for bos in [False, True]:
        for eos in [False, True]:
            backend = Tokenizer(
                models.WordLevel({"[UNK]": 0, "<s>": 1, "</s>": 2, "Hello": 3}, unk_token = "[UNK]")
            )
            backend.pre_tokenizer = pre_tokenizers.WhitespaceSplit()
            single = ("<s> " if bos else "") + "$A" + (" </s>" if eos else "")
            backend.post_processor = processors.TemplateProcessing(
                single = single, special_tokens = [("<s>", 1), ("</s>", 2)]
            )
            tok = PreTrainedTokenizerFast(
                tokenizer_object = backend,
                unk_token = "[UNK]",
                bos_token = "<s>" if bos else None,
                eos_token = "</s>" if eos else None,
            )
            for template in [
                "Hello",
                ("<s> " if bos else "") + "Hello" + (" </s>" if eos else ""),
                " \nHello",
            ]:
                ...
                if args.label == "fixed":
                    assert (
                        value["ids"] == tok(value["prompt"], add_special_tokens = False)["input_ids"]
                    )
                control_count += 1
    success = has_template_bos and all(c["matches_template_tokens"] for c in cases)
    result = {
        "label": args.label,
        "raw_ids": raw,
        "direct": direct,
        ...
        "versions": {
            n: proof.importlib.metadata.version(n)
            for n in ["transformers", "tokenizers", "torch", "unsloth-zoo"]
        },
        "failed_assertion": None
        if success
        else (
            "chat_template_missing_bos"
            if not has_template_bos
            else "rendered_chat_matches_template_tokens"
        ),
    }
...
def paired(args):
    ...
    for label, sha in [
        ...
        if sha and not repo.exists():
            subprocess.run(
                ["git", "worktree", "add", "--detach", str(repo), sha], cwd = root, check = True
            )
        if sha:
            assert (
        ...
        results[label] = r
        assert proc.returncode == (1 if label in {"pr", "latest"} else 0), r
    ...
    assert results["latest"]["exported"]["ids"] == results["fixed"]["exported"]["ids"]
    assert results["latest"]["failed_assertion"] == "chat_template_missing_bos"
    assert results["fixed"]["template_ids"] == results["baseline"]["template_ids"]
    ...
    assert (
        results["baseline"]["versions"]
        == results["pr"]["versions"]
        == results["latest"]["versions"]
        == results["fixed"]["versions"]
    )
    (output / "paired-result.json").write_text(json.dumps(results, indent = 2), encoding = "utf-8")
    summary = "Main PASS, original PR duplicate-BOS FAIL, latest PR missing-template-BOS FAIL, candidate PASS. Raw completion BOS preserved; 20 real Gemma chat cases and 12 tokenizer policy controls per revision."
    print(summary)
