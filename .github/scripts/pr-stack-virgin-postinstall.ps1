...
try {
    # Exercise the exact policy-safe trampoline generated by install.ps1. The
    # desktop uses the same python -c bootstrap; python -m imports the package
    # before __main__ can rewrite argv[0], so it is not the managed launch shape.
    & $cmdShim --version
    if ($LASTEXITCODE -ne 0) { throw "policy-safe cmd shim failed from System32: $LASTEXITCODE" }
