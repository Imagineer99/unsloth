...
def load_boundary(repo, processor, config):
    ...
    calls = [n for n in ast.walk(tree(path)) if isinstance(n, ast.Assign)
             and isinstance(n.value, ast.Call) and isinstance(n.value.func, ast.Name)
             and n.value.func.id == "_apply_post_load_tokenizer_fixes"]
    if not calls:
        assert not any(isinstance(n, ast.FunctionDef) and n.name == "_apply_post_load_tokenizer_fixes"
                       for n in tree(repo / "unsloth/tokenizer_utils.py").body)
        return processor
    assert len(calls) == 1
    utilities = repo / "unsloth/tokenizer_utils.py"
    names = {"_tokenizer_objects", "_tokenizer_stored", "_normalize_type_name",
             "_is_gemma4_config", "_is_gemma4_tokenizer", "_chat_template_emits_bos",
             "_is_gemma4_instruct_tokenizer", "_needs_gemma4_base_bos",
             "_has_add_bos_token_setter", "_update_generic_fast_post_processor",
             "_enable_add_bos_token", "_fix_gemma4_base_bos_token",
             "_apply_post_load_tokenizer_fixes"}
    scope = {"_GEMMA4_INSTRUCT_EOS": "<turn|>"}
    definitions(utilities, names, scope)
    scope.update(tokenizer=processor, auto_config=config, fix_tokenizer=True,
...
def attach_template(repo, processor):
    ...
    template = next(ast.literal_eval(n.value) for n in tree(path).body
                    if isinstance(n, ast.Assign) and any(isinstance(t, ast.Name)
                    and t.id == "gemma4_template" for t in n.targets))
    chat = module("unsloth.chat_templates", {
        "ProcessorMixin": ProcessorMixin, "logger": logging.get_logger("bos-proof"), "re": re,
        "CHAT_TEMPLATES": {"gemma-4": (template, "<turn|>", False, None)},
        "DEFAULT_SYSTEM_MESSAGE": {"gemma-4": None},
    })
    definitions(path, {"get_chat_template", "_change_system_message", "_escape_jinja_literal"}, chat.__dict__)
...
def studio_encode(repo, tokenizer, prompt):
    ...
    method = next(n for n in ast.walk(tree(path)) if isinstance(n, ast.FunctionDef)
                  and n.name == "generate_stream")
    nodes = [n for n in ast.walk(method) if isinstance(n, ast.Assign)
...
def probe(args):
    ...
    candidate = tok(prompt, add_special_tokens=tok.bos_token is None or not prompt.startswith(tok.bos_token))["input_ids"]
    ...
    result = {
        "label": args.label, "source_sha": subprocess.check_output(["git", "rev-parse", "HEAD"], cwd=repo, text=True).strip(),
        "model": MODEL, "revision": REVISION, "tokenizer_sha256": TOKENIZER_SHA256,
        "versions": {n: importlib.metadata.version(n) for n in ["transformers", "tokenizers", "torch", "torchvision", "unsloth-zoo"]},
        "source_hashes": {p: source_hash(repo / p) for p in ["unsloth/tokenizer_utils.py", "unsloth/models/vision.py", "unsloth/chat_templates.py", "studio/backend/core/inference/inference.py"]},
        "zoo_pad_source_hash": source_hash(Path(importlib.metadata.distribution("unsloth-zoo").locate_file("unsloth_zoo/pad_token.py"))),
        "prompt": prompt, "raw_ids": raw_ids, "direct_ids": direct_ids,
        "exported_studio_ids": ids, "candidate_boundary_ids": candidate,
        "exported_chat_has_one_bos": passes,
        "failed_assertion": None if passes else "exported_chat_has_one_bos",
    }
    (output / "result.json").write_text(json.dumps(result, indent=2), encoding="utf-8")
    print(json.dumps(result, indent=2), flush=True)
    print(("PASS" if passes else "REGRESSION_ASSERTION_FAILED") + " exported_chat_has_one_bos", flush=True)
...
def paired(args):
    ...
    output.mkdir(parents=True, exist_ok=True)
    snapshot = args.snapshot or snapshot_download(MODEL, revision=REVISION, cache_dir=output / "hf-cache",
    ...
    for label, sha in [("baseline", BASE), ("pr", HEAD)]:
        ...
        if not result_file.exists():
            raise RuntimeError(f"{label}: setup/execution failed, NOT regression proof; see {label}.log")
        values[label] = json.loads(result_file.read_text(encoding="utf-8"))
        values[label]["returncode"] = completed.returncode
        print(f"{label}: rc={completed.returncode}, raw={values[label]['raw_ids']}, exported={values[label]['exported_studio_ids'][:5]}", flush=True)
    ...
    assert a["source_hashes"]["studio/backend/core/inference/inference.py"] == b["source_hashes"]["studio/backend/core/inference/inference.py"]
    assert a["source_hashes"]["unsloth/chat_templates.py"] == b["source_hashes"]["unsloth/chat_templates.py"]
