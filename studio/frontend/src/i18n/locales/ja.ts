  ...
  settings: {
    ...
    resources: {
      ...
      modelMemory: {
        ...
        mlockNotApplicable: "読み込み済みのモデルはすべて GPU 上にあるため、システム RAM に固定すべきコピーがなく --mlock は省略されます。「モデルを GPU メモリに保持」は引き続き有効で、アイドル時の自動アンロードは無効のままです。",
        mlockUngoverned: "読み込み済みのモデルは、「モデルメモリ」がメモリ配置フラグを制御しないランナーを使用しています。「常駐させる」は引き続き画像および動画モデルのアイドル時の自動アンロードを無効にします。llama.cpp のメモリポリシーは、次に読み込む llama.cpp モデル用に保存されたままです。",
