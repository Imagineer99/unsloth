...
def test_profile_hardening_precedes_every_use_it_protects():
    ...
    for marker, what in (
        ...
        ("& $VenvPython -m unsloth_cli @studioArgs", "the setup handoff"),
    ):
...
def test_the_handoff_is_published_only_around_the_setup_child():
    ...
    call = _locate(source, "& $VenvPython -m unsloth_cli @studioArgs", "the child invocation")
...
def test_the_proxy_handoff_does_not_outlive_the_installer():
    ...
    child = installer.index("& $VenvPython -m unsloth_cli @studioArgs")
